...
document.addEventListener("keyup", (e)=>{
    ...
    if (moves[e.key]) moves[e.key].call(game);
    updateGameField();
    setScore();
    setMessage();
});
...
function setScore() {
    if (gameScore) gameScore.textContent = game.getScore();
}
function setMessage() {
...
class Game {
    ...
    checkStatus() {
        ...
        for(let row = 0; row < 4; row++)for(let col = 0; col < 4; col++){
            ...
            if (cellValue === 2048) {
                if (this.status !== Game.gameStatus.win) this.status = Game.gameStatus.win;
            }
        ...
        if (!hasEmptyCells && !canMove) this.status = Game.gameStatus.lose;
    }
    moveLeft() {
